...
func GetAd(ctx *gin.Context) {
	...
	ctx.IndentedJSON(http.StatusOK, AdResponse{
		...
		ImpressionUrl:     fmt.Sprintf("%s/impression/%s", config.Config().Server.EventSeverExternalHost, helper.RandStr(config.Config().Server.OTLlength)),
		ClickUrl:          fmt.Sprintf("%s/click/%s", config.Config().Server.EventSeverExternalHost, helper.RandStr(config.Config().Server.OTLlength)),
